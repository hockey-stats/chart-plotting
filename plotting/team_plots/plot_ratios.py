...
def main():
    ...
    xg_plot = TeamPlot(dataframe=base_df, filename='xg_ratios.png', x_column='xGFph', y_column='xGAph', 
                       title='Team xG Rates (5v5, Score, Venue, and Flurry adjusted)', 
    ...
    # Plot Team Corsi ratios
    g_plot = TeamPlot(dataframe=base_df, filename='corsi_ratios.png', x_column='CFph', y_column='CAph', 
                      title='Team Shot Attempt Rates (5v5)', x_label='Shot Attempts For per hour', 
                      y_label='Shot Attempts Against per hour (inverted)',
                      ratio_lines=True, invert_y=True, plot_x_mean=True, plot_y_mean=True, 
                      quadrant_labels=(('DULL', 'GOOD'), ('BAD', 'FUN')))
    g_plot.make_plot()
